	...
	*/
	...
	if ( ! document.getElementById('ASTEROIDS-NAVIGATION') ) {
		...
		with ( this.navigation.style ) {
			...
			background = '#fff';
			color = '#222';
			padding = '8px 12px'; // 增加内边距，确保背景完全覆盖分数
		...
		with ( this.points.style ) {
			font = '24pt Arial, sans-serif'; // 稍微减小字体大小
			fontWeight = 'bold';
			position = 'relative';
			left = '0px'; // 重置位置，因为背景已经足够大
			color = '#222'; // 确保字体颜色
		}
		this.points.className = "ASTEROIDSYEAH";
		this.navigation.appendChild(this.points);
		
		// highscore link - 注释掉广告链接
		/*
