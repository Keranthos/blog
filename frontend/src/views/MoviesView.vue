<template>
  <div class="media-view">
    ...
    <div class="media-list" @scroll="handleScroll">
      <MediaCard v-for="media in mediaList" :key="media.ID" :media="media" type="movies" @media-deleted="removeMedia" />
    </div>
  </div>
</template>
...
const loadMedia = async () => {
  ...
  try {
    if (mediaCache.value[currentPage.value]) {
      ...
      const response = await getMediaList('movies', currentPage.value)
